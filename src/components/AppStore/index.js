...
class AppStore extends Component {
  ...
  clickTabItem = tabId => {
    this.setState({changeAccTab: tabId})
  }

  render() {
    const {searchInput, changeAccTab} = this.state
    const changedAppList = appsList.filter(each =>
      each.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    const changedAppListAccTab = changedAppList.filter(
      each => each.category === changeAccTab,
    )
    return (
      <div className="cont">
        <h1>App Store</h1>
        ...
        <ul className="tabItem">
          {tabsList.map(each => (
            <TabItem
              ...
              isActive={changeAccTab === each.tabId}
            />
          ))}
        </ul>
        <ul className="appItem">
          {changedAppListAccTab.map(each => (
